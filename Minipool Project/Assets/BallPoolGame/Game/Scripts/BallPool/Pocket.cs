...
//        }
        IEnumerator RollTheBall(BallRoll roll)
        {
            ...
            {
                roll.ball.body.position = QuadraticCurve.CalculateValue(time / length, nodes);
            ...
            {
                roll.ball.body.position = QuadraticCurve.CalculateValue(0.99f * roll.maxLength / length, nodes);
                physicsManager.CallBallMove(roll.ball.id, roll.ball.body.position, Vector3.zero, Vector3.zero);
            }
            yield return new WaitForFixedUpdate();
        }
