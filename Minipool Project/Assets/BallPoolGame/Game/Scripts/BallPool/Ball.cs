...
namespace BallPool.Mechanics
{
    public class Ball : MonoBehaviour
    {
        ...
        public struct MechanicalState
        {
            ...
            {
                return "[" + state.time + ";" + state.pocketId + ";" + state.hitShapeId + ";" + DataManager.Vector3ToString(state.position) + "; " +  DataManager.Vector3ToString(state.velocity) + "; " +  DataManager.Vector3ToString(state.angularVelocity) + "]";
            }
            public static MechanicalState StateFromString (string state)
            {
                ...
                int pocketId = int.Parse(values[1]);
                int hitShapeId = int.Parse(values[2]);
...
//            return false;
        ...
        public string mechanicalStateData
        {
            get
            {
                MechanicalState state = new MechanicalState(DataManager.CutValue(listener.physicsManager.moveTime), pocketId, hitShapeId, position, listener.body.velocity, listener.body.angularVelocity);
...
//                    ballShadow.position = CalculateBallShadowPosition();
        ...
        IEnumerator WaitAndPlayBallInPocket()
        {
			...
			if (gameObject.GetComponent<Ball> ().id != 0 && gameObject.GetComponent<Ball> ().id != 1) {
