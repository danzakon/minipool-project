...
public class dans_Network : NetworkEngine
{
    private PhotonView photonView;
    private string gameVersion = "1.01";
    private PhotonPlayer opponentPlayer;
    private MinipoolNetworkMessenger messenger;

    public void Initialize()
    {
        if (!messenger)
        {
            messenger = gameObject.AddComponent<MinipoolNetworkMessenger>();
        }
    }
    ...
    public override void OnGoToPlayWithPlayer(PlayerProfile player)
    {
        ...
        else
        {
            ...
            photonView.RPC("OnOpponentReadyToPlay", opponentPlayer, NetworkManager.PlayerToString(NetworkManager.mainPlayer));
        }
    }
    ...
    public void OnJoinedRoom()
    {
        if (PhotonNetwork.player != PhotonNetwork.masterClient)
        {
            ...
            photonView.RPC("Dan's OnOpponentReadyToPlay", opponentPlayer, NetworkManager.PlayerToString(NetworkManager.mainPlayer));
        }
    ...
    {
        Debug.Log("A new player connected to dans network" + newPlayer.ID);
    ...
    void OnConnectedToPhoton()
    {
        ...
        Debug.Log("Dan's network state " + state);
    }

    void OnMasterClientSwitched(PhotonPlayer newMasterClient)
    {
        Debug.LogWarning("Dan's OnMasterClientSwitched");
    }
    ...
    public override void Connect()
    {
        ...
        {
            Debug.Log("Dan's photonView " + photonView + "  reachable " + reachable + "   PhotonNetwork.connectionStateDetailed " + PhotonNetwork.connectionState);
    ...
    {
        Debug.LogWarning("Dan's OnPhotonPlayerDisconnected");
        PhotonNetwork.LeaveRoom();
    ...
    public IEnumerator printDansNetworkInfo()
    {
        while (true)
        {
            yield return new WaitForSeconds(3.0f);
            Debug.Log(NetworkManager.players);
        }
    }
    ...
    public override void OnOpponentReadyToPlay(string playerData)
    {
        ...
        Debug.Log("Dan's OnOpponentReadyToPlay " + playerData);
        ...
        if (PhotonNetwork.player != PhotonNetwork.masterClient)
        {
            ...
            photonView.RPC("Dan's OnOpponenStartToPlay", opponentPlayer, turnIdForSend);
        }
    }
    ...
    {
        Debug.Log("Dan's OnOpponentStartToPlay " + turnId);
    ...
    {
        StartCoroutine(messenger.OnOpponentInGameScene());
    }
    [PunRPC]
